...
const Register = () => {
 ...
 const handleRegister = (e) => {
  ...
  return (
    <>
    {loading ? ( 
      <div className='flex justify-center items-center h-screen'><ClipLoader
      color="#0a69ae"
     size={45}
        speedMultiplier={0.5}/>
        </div>
...
<div className='mt-3'>
  <button className='ml-3 mr-5 font-semibold'>Forgot Password?</button>
  </div>
      </div>*/} 
      <div className='ml-2 p-6 flex flex-col gap-y-6'>
        <Link to="/">
        <button type='submit' className='border-2 border-indigo-700 bg-indigo-700 font-medium text-white py-1 px-5 w-full rounded-md hover:bg-transparent hover:text-indigo-700'>Sign In</button>
        </Link>
